...
round(sum(
	case
		...
		when po.currency = 'GBP' then (po.negotiated_unit_price * po.unit_price) * 4.97
...
),2) as average_supplier_spend
from suppliers s 
join purchase_orders po 
on s.supplier_id = po.supplier_id 
group by s.supplier_id, s.supplier_name
order by average_supplier_spend desc;
...
round(avg(po.actual_lead_time_days)) as average_lead_time
from purchase_orders po 
...
pb.order_count,
pb.late_order_count,
round(pb.late_order_count * 100 / nullif(pb.order_count,0),2) as percentage_of_late_orders,
pb.total_supplier_spend,
pb.total_supplier_savings,
pb.average_lead_time,
case 
	when pb.total_supplier_spend > 300000 and pb.late_order_count * 100 / nullif(pb.order_count, 0) >= 65 then 'High Risk'
	when pb.total_supplier_spend > 200000 or pb.late_order_count *100 / nullif(pb.order_count,0) >= 20 then 'Medium Risk'
	else 'Low Risk'
end as risk_factor
from performance_base pb
join suppliers s
on pb.supplier_id = s.supplier_id 
group by pb.supplier_id, s.supplier_name, pb.order_count, pb.late_order_count, pb.total_supplier_spend, pb.total_supplier_savings, pb.average_lead_time
order by pb.total_supplier_spend desc
)
